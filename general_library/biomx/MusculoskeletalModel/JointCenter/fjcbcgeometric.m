function [c1,r] = fjcbcgeometric(r1,p1,m2,c0)
%   
%   functional joint center estimator: bias compensated geometric method, 
%   piazza et al. 2001, Ehrig et al. 2006, Shakarji 1998, Pratt 1987
%
...
%   to find solution. Cost function slightly different in that the squared
%   error is divided by the squared radius to compensate for the same bias
%   in the algebraic method (see Ehrig et al. 2006, calls this the pratt
%   method)
%
%----------------------------------INPUTS----------------------------------
%
...
% eq 3 in Ehrig et al. 06
% also ref Shakarji 1998 section 3.3
...
for i = 1:length(imkr)
    ...
    % marker relative to joint center
    d1 = u(1,i) - c(1);
    d2 = u(2,i) - c(2);
    d3 = u(3,i) - c(3);
    
    % length^2: center to marker
    mag2 = d1^2 + d2^2 + d3^2;
    
    % pratt objective
    err(i) = mag2/r(imkr(i)) - r(imkr(i));
    
    % derivs
    jac(i,imkr(i)) = -mag2/r(imkr(i))/r(imkr(i)) - 1;
    jac(i,end-2) = -2/r(imkr(i)) * d1;
    jac(i,end-1) = -2/r(imkr(i)) * d2;
    jac(i,end) = -2/r(imkr(i)) * d3;
end
