function [c2] = fjclmpbayes(r1,r2,p1,m2,c2lb,c2ub,nseed,maxiter)

%
%   functional joint center estimator: least moving point, Marin et al.
%   2003, Ehrig 07, Bayesian optimization
%
...
%
%   nseed:
%       NumSeedPoints in bayesopt()
%
%   maxiter:
%       MaxObjectiveEvaluations argument in bayesopt() function
%
%---------------------------------OUTPUTS----------------------------------
%
%   c2:
...
cX = optimizableVariable('cX',[c2lb(1),c2ub(1)],'Type','real');
cY = optimizableVariable('cY',[c2lb(2),c2ub(2)],'Type','real');
cZ = optimizableVariable('cZ',[c2lb(3),c2ub(3)],'Type','real');
c2 = bayesopt(@objfun,[cX,cY,cZ],'NumSeedPoints',nseed,'MaxObjectiveEvaluations',maxiter);
c2 = table2array(c2.XAtMinObjective)';
